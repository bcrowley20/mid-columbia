...
Status: draft v13 — **Phases 0–6 complete**, plus a Render deployment (§16) and a Windows compatibility pass (§17), both done outside the phase sequence; later phases sketched and open to revision as we build.
...
- **Per-point value tips on hover**: the user asked for "little tips at each of the intersection points" as the cursor moves along the lines — uPlot's own hover affordances stop at a crosshair and (if `legend.live` were still on) a value column back in the header, neither of which sits at the actual point on the line. `chart.ts` now creates one small floating label per series (`.chart-point-tip`, styled in `style.css`), positioned every `hooks.setCursor` firing via `u.valToPos(...)` at that series' own x/y for the hovered index — shown only when the series is currently toggled on and has a real (non-null) value there, hidden otherwise (including whenever the cursor leaves the chart, `u.cursor.idx == null`). Verified in a real browser: hidden before any hover; correct value/unit and unique screen position per series on hover; positions and values update as the cursor moves along the line; all hide together when the cursor leaves the plot; respects the water/air-temperature checkboxes (unchecking a series' checkbox hides its tip even mid-hover). With only the two default depth series shown it's clean; with all seven series (both temperature toggles on) two unrelated series can land at a similar screen height and their tips visually crowd each other — still distinguishable by each tip's colored border matching its line and legend chip, and only happens in that all-series-on edge case, not the default view. Zero console errors throughout.
- **Distinguishable colors for multiple wells of the same type**: the user pointed out that Site 3 (Carlson's only site with two GW wells) rendered both GW 3a and GW 3b in what read as "the same blue," needing the legend to tell them apart. The original `GW_SHADES`/`IS_SHADES` arrays in `chart.ts` (§12) *were* technically three different hex values each, but all close mid-tones of the same lightness — not visually distinct enough at line-width scale. Replaced each with a base/dark/light triad (`#2563eb`/`#1e3a8a`/`#38bdf8` for GW, `#059669`/`#14532d`/`#4ade80` for IS) chosen for contrast, not just hue. Index 0 — what a site with only one well of that type gets — is unchanged from before, so single-well sites (every Carlson site except Site 3) look exactly as they did. The per-well `color` was already shared between that well's depth and temperature series (`fetchWellSeries` passes the same `color` into both), so "same colors for the temp lines" the user asked to preserve was already correct and needed no change — confirmed, not assumed, by reading each legend chip's actual inline `border` color in a real browser for both a single-well site (Site 1, unchanged base colors) and Site 3 (GW 3a base blue, GW 3b navy, each well's water-temp chip matching its own depth chip exactly). Zero console errors.
...
- **Phase 7 — done.** Polish pass: audited every `Optional`/`None`-returning function in `src/midcolumbia` (`catalog.find_*`, `find_well_by_device_serial`, `scanner.find_handler`, `db.latest_reading_timestamp`, `water_depth._nearest_reading`, `routes_wells._project_owning_well`, `routes_readings._as_utc`) against every call site, and every `except` clause, for a case that's caught but not actually surfaced. Found and fixed two real gaps:
  - `routes_projects.py::update_site` looked up the owning project via a bare `next(... )` with no default - unlike every other "find the project that owns this entity" lookup in the API (`routes_wells.py`'s `_project_owning_well` callers), which all use `next((...), None)` + an explicit 404. Currently unreachable in practice (the reach a site resolves to always came from the same `catalogs` list), but an unhandled `StopIteration` surfacing as an opaque 500 instead of the app's normal 404 would have been a real trap if that ever stopped being true. Now matches the established pattern.
  - `ChartPanel.open()`/`openAtm()` (`web/src/chart.ts`) left the panel stuck on "Loading…" forever if a readings fetch failed - the rejection only ever reached `console.error` via `main.ts`'s `onSelectSite`/`onSelectAtm`, with nothing visible in the UI. This one *is* reachable (any backend hiccup while a chart is open) and was a real "error not handled" gap, not just a defensive nicety. Fixed by wrapping both methods' bodies in `try`/`catch` with a new `showLoadError()` that renders into `#chart-panel-body` using the same `#chart-panel-empty` convention already used for "no readings yet" - confirmed live by aborting `/api/wells/readings` mid-request via Playwright's `page.route()` and watching both the site and ATM chart panels show `"Failed to load chart data: ..."` instead of hanging, with a clean rest-of-page and the error still logged to console for debugging.

  No bare/silent `except` blocks or swallowed frontend promise rejections found elsewhere - every other `except` already re-raises as a domain error (`ManagementError`/`CatalogError`/`SettingsError`/`ParseError`) or is the one deliberate, already-documented case (`app.py`'s startup-ingest handler logging and continuing rather than failing boot), and every other frontend `.catch()` already reaches either the error banner (`showError`) or an `alert()`. No dead code, stale comments, or leftover `TODO`/`FIXME` markers found. `uv run pytest` (112 passing, unchanged - the `update_site` fix is defensive/currently-unreachable so no new test was worth contriving for it) and `tsc --noEmit` both clean; the chart fix verified live per the above rather than via a committed test, since §15 already notes there's no frontend test suite to add it to.

  §15's "open items to revisit" and further graphing/plotting work (year-over-year comparisons, moving averages, etc.) were explicitly left for the user to pick back up separately - not part of this pass.
...
## 17. Windows compatibility — done

Requested directly (not implied by any prior phase): the app should also run on a Windows machine, not just macOS/Linux (where it's actually been developed and verified all along). This was a **code and configuration review**, not a from-scratch build — nothing here needed new functionality, only checking existing choices for platform assumptions and fixing the ones that were real.

**Reviewed and already fine, no changes needed**:
- Every file path in the backend goes through `pathlib.Path`, never a hardcoded `/`-joined string (`config.py`, `catalog.py`, `management.py`, `ingestion/scanner.py`, `storage/db.py`) — `pathlib` resolves separators correctly per-OS on its own.
- The `_slug_path` ids in `catalog.py` (e.g. `carlson-creek-restoration/lower-stream/site-1`) use `/` as a literal character in an opaque string id, not as a filesystem separator — safe on Windows for the same reason a URL path is.
- Both ingestion handlers match file extensions case-insensitively (`path.suffix.lower() == ".csv"` / `".xlsx"`) — matters because Windows' filesystem is case-insensitive by default and files may arrive with an uppercase extension.
- No Unix-only stdlib module (`fcntl`, `pwd`, `termios`, ...) or shell-specific `subprocess`/`os.system` call anywhere in `src/midcolumbia`.
- `uv`, Node/npm, and the `npm run build` script (`tsc && vite build`) are all natively cross-platform — npm runs scripts through `cmd.exe` on Windows, which also understands `&&`, so no script rewrite was needed.
- The longest real file path in `data/` is 149 characters relative to the repo root — nowhere near Windows' legacy 260-character `MAX_PATH` limit even accounting for a typical clone location.
- No filename in the real Carlson dataset uses a character Windows forbids (`< > : " | ? *`) or a reserved device name.

**One real, concrete risk found and fixed**: nothing in the repo declared which tracked files are binary. Git's own heuristic (sniffing for a NUL byte) is usually right, but wasn't guaranteed — and if it ever misfired on `.xlsx` (a zip archive `openpyxl` parses byte-for-byte) under Windows' commonly-recommended `core.autocrlf=true`, the CRLF conversion would silently corrupt every IS well's real data on checkout, with no error until ingestion inexplicably failed or mis-parsed. Added `.gitattributes` explicitly marking `*.xlsx`, `*.hobo`, `*.png`, and `*.webp` as `binary`, removing any dependence on the heuristic. (The CSV handler was already unaffected by this either way — `file` confirms the real CSVs are natively CRLF already, from the HOBOware Windows export tool, and `hoboware_csv.py` reads them with `encoding="utf-8-sig", newline=""` into `csv.reader`, which is the standard Python idiom for being agnostic to `\r\n` vs `\n` — not a fix made for this pass, but confirms the parser was already built correctly for Windows-originated files.)

**Documentation gap fixed while here**: `README.md` was a one-line stub (`See Project Description.md`) with no setup instructions for *any* OS, let alone Windows specifically. Rewrote it with prerequisites and exact run commands for macOS/Linux and Windows side by side (`winget install astral-sh.uv OpenJS.NodeJS.LTS` as the Windows equivalent of `brew install uv node`) — the commands themselves (`uv run midcolumbia-serve`, `npm install`, `npm run dev`, `npm run build`) are identical across all three, so this was about filling in what didn't exist yet, not branching logic per OS.

**Honestly disclosed, not verified**: no Windows machine or VM was available in this environment, so none of the above was confirmed by actually running the app on Windows — every other phase in this plan was verified that way (real browser, real Docker container, etc.), and this one is the exception. The review above is thorough static analysis, not a substitute for someone actually cloning the repo and running it on real Windows hardware at least once. If something doesn't work as described, that's the first place to look, and it's called out as such in the new README section.
